...
c        vmag  = sqrt((ppiclf_rprop(PPICLF_R_JUX,i)
c    >                 -ppiclf_y(PPICLF_JVX,i))**2
c    >               +(ppiclf_rprop(PPICLF_R_JUY,i)
c    >                 -ppiclf_y(PPICLF_JVY,i))**2)
c        dp    = ppiclf_rprop(PPICLF_R_JDP,i)
c        rep   = vmag*dp*rhof/rmu
c        rphip = ppiclf_rprop(PPICLF_R_JPHIP,i)
c        rphif = 1.0-rphip

c        beta1 = 150.0*rphip*rphip*rmu/rphif/dp**2 
c    >          +1.75*rphip*rhof*vmag/dp
c        reps = rep*rphif
c        reps = max(reps,1E-1)
c        if (reps .le. 1E3) then
c           cds = 24.0/reps*(1.0+0.15*reps**(0.687))
c        else
c           cds = 0.44
c        endif
c        beta2 = 0.75*cds*rphif*rhof*vmag*rphif**(-2.65)/dp
c   
c        rs   = 0.2
c        rdum = atan(150.0*1.75*(rphip - rs))/rpi + 0.5
c        beta = rdum*beta1 + (1.0-rdum)*beta2

c        rdum = ppiclf_rprop(PPICLF_R_JVOLP,i)*beta/rphip

c        fqsx = rdum
c    >          *(ppiclf_rprop(PPICLF_R_JUX,i)-ppiclf_y(PPICLF_JVX,i))
c        fqsy = rdum
c    >          *(ppiclf_rprop(PPICLF_R_JUY,i)-ppiclf_y(PPICLF_JVY,i))
